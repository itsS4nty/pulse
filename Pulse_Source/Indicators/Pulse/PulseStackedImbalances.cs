...
namespace NinjaTrader.NinjaScript.Indicators.Pulse
{
	public class PulseStackedImbalances : Indicator
	{
		...
		public string AskImbalanceColorSerializable
		{
			get
			{
				return AskImbalanceColor.ToString();
			}
			set
			{
		...
		public string BidImbalanceColorSerializable
		{
			get
			{
				return BidImbalanceColor.ToString();
			}
			set
			{
		...
		protected override void OnStateChange()
		{
			if (State == State.SetDefaults)
			{
				...
				ImbalanceVolume = 30;
				LineTillTouch = true;
				AskImbalanceColor = System.Windows.Media.Color.FromRgb(107, 111, 204);
				BidImbalanceColor = System.Windows.Media.Color.FromRgb(74, 74, 74);
				LineWidth = 2;
			...
			else if (State == State.DataLoaded)
			{
				...
				Print("======================================");
				Print("Pulse Stacked Imbalances LOADED");
				Print($"Ratio: {ImbalanceRatio}% | Range: {ImbalanceRange} levels | MinVol: {ImbalanceVolume}");
				Print($"Days Lookback: {DaysLookBack} | Line Duration: {PrintLineForXBars} bars");
				Print($"Historical Reconstruction: {EnableHistoricalReconstruction} | Opacity: {StackedImbalanceOpacity:P0}");
				Print("======================================");
			}
		...
		protected override void OnBarUpdate()
		{
			...
			else
			{
				...
				if (IsFirstTickOfBar && CurrentBar % 5 == 0 && activeImbalances.Count > 0)
				{
					...
					Print(string.Format("[{0}] Active zones: {1} Ask | {2} Bid", Time[0].ToString("HH:mm:ss"), num, num2));
				}
				if (LineTillTouch)
				{
		...
		protected override void OnMarketData(MarketDataEventArgs e)
		{
			...
			{
				if (e.MarketDataType != MarketDataType.Last || State == State.Historical || CurrentBar < 0)
				{
					return;
				}
		...
		private void DrawImbalanceZone(StackedImbalance imbalance, int endBarIndex)
		{
			...
			if (num >= 0)
			{
				...
				Draw.Line(this, imbalanceTag + "_top", isAutoScale: true, num, imbalance.TopPrice, num2, imbalance.TopPrice, brush, DashStyleHelper.Solid, width);
				Draw.Line(this, imbalanceTag + "_bottom", isAutoScale: true, num, imbalance.BottomPrice, num2, imbalance.BottomPrice, brush, DashStyleHelper.Solid, width);
			}
		}
		...
		private SolidColorBrush CreateZoneBrush(Color baseColor, double opacity, double minOpacity)
		{
			...
			SolidColorBrush solidColorBrush = new System.Windows.Media.SolidColorBrush(System.Windows.Media.Color.FromArgb((byte)Math.Max(10, Math.Min(255, (int)Math.Round(255.0 * num))), baseColor.R, baseColor.G, baseColor.B));
		...
		{
			return string.Format(CultureInfo.InvariantCulture, "{0}|{1}|{2:0.########}|{3:0.########}", barIndex, isAsk ? "A" : "B", topPrice, bottomPrice);
		}

		private string GetImbalanceTag(StackedImbalance imbalance)
		{
			return string.Format(CultureInfo.InvariantCulture, "StackZone_{0}_{1}_{2:0.#####}_{3:0.#####}", imbalance.BarIndex, imbalance.IsAskImbalance ? "A" : "B", imbalance.BottomPrice, imbalance.TopPrice);
		}
